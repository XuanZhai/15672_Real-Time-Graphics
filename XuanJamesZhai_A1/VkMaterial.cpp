 ...
 */
void VkMaterial_PBR::CreateDescriptorSets(const VkDevice& device, VkSampler const &textureSampler, const std::vector<VkImageView>& cubeMaps, const VkImageView& brdfLUT){
    /* Create one descriptor set for each frame in flight */
    std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, VKMDescriptorSetLayout);
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        descriptorWrites[0].dstArrayElement = 0;
        descriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        descriptorWrites[0].descriptorCount = static_cast<uint32_t>(brdfInfo.size());
        descriptorWrites[0].pImageInfo = brdfInfo.data();
        ...
        descriptorWrites[1].descriptorCount = static_cast<uint32_t>(cubeMapInfo.size());
