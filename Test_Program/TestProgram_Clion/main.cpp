...
private:
    ...
    VkBuffer vertexBuffer1 = VK_NULL_HANDLE;

    /* Handle to the memory of the vertex buffer */
    VkDeviceMemory vertexBufferMemory = VK_NULL_HANDLE;

    VkDeviceMemory vertexBufferMemory1 = VK_NULL_HANDLE;
    ...
    */
    void CreateVertexBuffer()
    {
        ...
        VkDeviceSize bufferSize1 = sizeof(vertices1[0]) * vertices1.size();

        /* Create a staging buffer as temporary buffer and use a device local one as actual vertex buffer. */
        VkBuffer stagingBuffer1;
        VkDeviceMemory stagingBufferMemory1;
        CreateBuffer(bufferSize1, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer1, stagingBufferMemory1);

        /* Copy the vertex data to the buffer using memcpy */
        void* data1;
        vkMapMemory(device, stagingBufferMemory1, 0, bufferSize1, 0, &data1);
        memcpy(data1, vertices1.data(), (size_t)bufferSize1);
        vkUnmapMemory(device, stagingBufferMemory1);

        /* The vertex buffer is now device local */
        CreateBuffer(bufferSize1, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer1, vertexBufferMemory1);

        /* Copy the buffer from the staging buffer to the device local vertex buffer */
        CopyBuffer(stagingBuffer1, vertexBuffer1, bufferSize1);
    ...
    */
    void UpdateUniformBuffer(uint32_t currentImage)
    {
        ...
        glm::mat4 vxx = glm::lookAt(glm::vec3(2,2,2),glm::vec3(0,0,0), glm::vec3(0,0,1));
        ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        //ubo.view = FromGLMtoXZM(vxx);
        ...
        if (swapChainExtent.width == 0 || (float)swapChainExtent.height == 0) {
            ubo.proj = glm::perspective(1.77777778f, 1.0f, 0.1f, 10.0f);
        }
        else {
            //ubo.proj = XZM::Perspective(1.777777778, swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
            ubo.proj = glm::perspective(1.777777778f, (float)swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
        }
    ...
    XZM::mat4 FromGLMtoXZM(const glm::mat4& nm){
    ...
    */
    void RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
        ...
        vkCmdDraw(commandBuffer, 504, 1, 0, 0);

        vertexBuffers[0] = { vertexBuffer1};
        offsets[0] = { 0};
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);

        vkCmdDraw(commandBuffer, 5370, 1, 0, 0);
    ...
    */
    void LoadModel()
    {
        std::ifstream inputfile("Models/sg-Articulation.Servo.b72", std::ios::binary);
        ...
        std::ifstream inputfile1("Models/sg-Articulation.Foot.b72", std::ios::binary);
        ...
        buffer << inputfile1.rdbuf();
        vertices1 = buffer.str();
    }
